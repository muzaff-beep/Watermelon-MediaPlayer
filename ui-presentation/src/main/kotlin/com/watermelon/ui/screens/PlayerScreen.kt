...
fun PlayerScreen(
    ...
    // PiP receiver — forward actions to controller.
            ...
            override fun onReceive(ctx: Context, intent: Intent) {
                when (intent.action) {
                    ...
                    PiPReceiver.ACTION_MUTE -> {
                        ...
                        currentVolume = vol
                        volumeFraction = vol.toFloat() / maxVolume
                    }
                    PiPReceiver.ACTION_PREV -> { /* next/prev needs queue — stub */ }
                    PiPReceiver.ACTION_NEXT -> { /* next/prev needs queue — stub */ }
                }
            }
        }
                    ...
                    ControlPanel(
                        ...
                        onPip = {
                            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
                                enterPiP(activity, isPlaying, currentVolume == 0, context)
                                isPiPEnabled    = true
                        ...
                        onBackground = {
                            if (!isBackgroundEnabled) {
                                context.startService(
                                    Intent(context, com.watermelon.playback.service.WatermelonPlaybackService::class.java)
                                )
...
private fun enterPiP(activity: Activity?, isPlaying: Boolean, isMuted: Boolean, context: Context) {
    activity ?: return
    val actions = mutableListOf<RemoteAction>()

    fun makeAction(action: String, iconRes: Int, title: String): RemoteAction {
    ...
    // Play/pause action using system drawable resources.
